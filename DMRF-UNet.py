...
import scipy.io as scio  # MATLAB dosyalarını okumak için
import matplotlib.pyplot as plt  # Grafik çizimleri için
import matplotlib.image as mpimg  # Görüntü işleme için
from PIL import Image  # Görüntü işleme ve dönüştürme için
from sklearn import preprocessing  # Veri ön işleme için
from sklearn.preprocessing import scale  # Veri ön işleme için
import tensorflow as tf  # Derin öğrenme kütüphanesi
from tensorflow import keras  # Yüksek seviye derin öğrenme kütüphanesi

# Proje parametreleri
N_class = 2  # Sınıf sayısı
N_train = [98, 98]  # Eğitim verisi sayısı
N_test = [98, 98]  # Test verisi sayısı
image_sizeX = 128  # Görüntü genişliği
image_sizeY = 128  # Görüntü yüksekliği
num_channel = 1  # Görüntü kanal sayısı
train_data = []  # Eğitim verisi
train_mask1 = []  # Eğitim maskesi 1
train_mask2 = []  # Eğitim maskesi 2
test_data = []  # Test verisi
test_mask1 = []  # Test maskesi 1
test_mask2 = []  # Test maskesi 2


# Veri setinin yüklenmesi ve uygun formata dönüştürülmesi
for i in range(N_class):
    for j in range(N_train[i]):
        x = mpimg.imread('dataset/data/%d.png'%(j+1))
        x = x.reshape(image_sizeX, image_sizeY, 1)
        train_data.append(x)
        m1 = mpimg.imread('dataset/mask1/%d.png'%(j+1))
        m1 = m1.reshape(image_sizeX, image_sizeY, 1)
        train_mask1.append(m1)
        m2 = mpimg.imread('dataset/mask2/%d.png'%(j+1))
...
def multi_scale(x, filters, padding="same", strides=1):
    # Çok ölçekli örnek çıkarma bloğu
...
def down_block2(x, filters, kernel_size=(3, 3), padding="same", strides=1):
    # Çok ölçekli alt örnek çıkarma bloğu 2
    f = int(filters/4)
    c = multi_scale(x, f)
    c = keras.layers.Conv2D(filters, kernel_size, padding=padding, strides=strides, activation="relu")(c)
    c = multi_scale(c, f)
...
def up_block2(x, skip, filters, kernel_size=(3, 3), padding="same", strides=1):
    # Çok ölçekli üst örnek çıkarma bloğu 2
    f = int(filters/4)
    us = keras.layers.UpSampling2D((2, 2))(x)
...
def DMRF_UNet():
    # Çift ölçekli çok katmanlı sinir ağı modeli
    f0 = 64
...
history = model.fit(x=train_data, y=[train_mask1, train_mask2], batch_size=batch_size, epochs=total_epoch, verbose=2, \
    validation_data=(test_data, [test_mask1, test_mask2]), callbacks=[model_checkpoint, lr_checkpoint])


# Eğitilmiş modelin test edilmesi ve sonuçların görselleştirilmesi.
...
for i in range(len(test_data)):
    ...
    pred1.convert('L').save('exp/visual/1pred_%d.png'%(i+1))
    pred2 = test_pred2[i].reshape(image_sizeX, image_sizeY) * 255
    pred2 = Image.fromarray(pred2)
    pred2.convert('L').save('exp/visual/2pred_%d.png'%(i+1))
